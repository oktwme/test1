...
public:
	...
	static void Render()
	{
		if (ImGui::BeginTabItem("Game"))
		{
			...
			static std::vector<std::pair<int, std::string>>instalockChamps = GetInstalockChamps();
			static std::vector<std::string>instalockChampsNames;
			...
			if (onOpen)
			{
				if (!instalockChamps.empty())
				{
					...
					selectedChamp = ChampIdToName(S.gameTab.backupId);
					...
					for (size_t i = 0; i < instalockChamps.size(); i++)
					{
						...
						if (backupPickComboData.input == instalockChamps[i].second)
							backupPickComboData.index = i;
					}
					instalockComboData.items = instalockChampsNames;
					backupPickComboData.items = instalockChampsNames;
				}
			}
			...
			if (ImGui::ComboAutoSelect("##comboBackupPick", backupPickComboData))
				if (backupPickComboData.index != -1)
					for (const auto& champ : champSkins)
